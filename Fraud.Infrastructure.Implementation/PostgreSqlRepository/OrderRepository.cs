    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task CreateOrder(Order order)
        {
            ...
            const string query = @"INSERT INTO orders (external_ref, user_id, amount, source, destination) 
                                   VALUES (@ExternalRef, @UserId, @Amount, @Source, @Destination);";
            await _dbConnection.ExecuteAsync(query, order);
        }

        public async Task<IEnumerable<Order>> GetOrdersBySource(string source, DateTimeOffset dateFrom, DateTimeOffset dateTo, int limit = 100)
        {
            if (_isDisposed)
                throw new ObjectDisposedException(nameof(OrderRepository));
            if(_dbConnection.State != ConnectionState.Open)
            ...
            return await _dbConnection.QueryAsync<Order>(query, new
            {
                Source = source,
                DateFrom = dateFrom,
                DateTo = dateTo,
        ...
        public async Task<IEnumerable<Order>> GetOrdersByDestination(string destination, DateTimeOffset dateFrom, DateTimeOffset dateTo, int limit = 100)
        {
            if (_isDisposed)
                throw new ObjectDisposedException(nameof(OrderRepository));
            if(_dbConnection.State != ConnectionState.Open)
